...
Last Modified By: 
*/
/* This Class is used represents Data Access Lafghfhyer for Content Module*/
...
namespace YPLMS2._0.API.DataAccessManager
{
    public interface IContentModuleRepository
    {
        ...
        ContentModule GetByIdForCoursePlayer(string clientId, string courseId);
    }
    /// <summary>
    /// class ContentModuleAdaptor
    /// </summary>
    public class ContentModuleAdaptor : IDataManager<ContentModule>, IContentModuleRepository,IContentModuleAdaptor<ContentModule>
    {
        #region Declaration
        ...
        public ContentModule GetContentModuleByID(ContentModule pEntContModule)
        {
            ...
            {
                _expCustom = new CustomException(_strMessageId, CustomException.WhoCallsMe(), ExceptionSeverityLevel.Critical, expCommon, true, sConnString);
                throw _expCustom;
            }
            finally
            {
        ...
        //CheckLockCourseAssessment
        public ContentModule CheckLockCourseAssessment(ContentModule pEntContModule)
        {
            _sqlObject = new SQLObject();
            ContentModule entContModule = new ContentModule();
            ContentModuleLanguages entCML = new ContentModuleLanguages();
            ...
            try
            {
                sConnString = _sqlObject.GetClientDBConnString(pEntContModule.ClientId);
                sqlConnection = new SqlConnection(sConnString);
                _sqlcmd = new SqlCommand(Schema.ContentModule.PROC_GET_LOCK_ASSESSMENTCOURSE, sqlConnection);
                _sqlpara = new SqlParameter(Schema.ContentModule.PARA_CONTENT_MODULE_ID, pEntContModule.ID);
                _sqlcmd.Parameters.Add(_sqlpara);
                if (!string.IsNullOrEmpty(pEntContModule.LanguageId))
                {
                ...
                _sqlreader = _sqlObject.SqlDataReader(_sqlcmd, false);
                _sqlreader.Read();
                _entContentModule = FillLockAssessmentCourse(_sqlreader);
            }
            catch (Exception expCommon)
            {
        ...
        //
        public ContentModule SearchContentModuleURL(ContentModule pEntContModule)
        {
            _sqlObject = new SQLObject();
            ContentModule entContModule = new ContentModule();
            string sConnString = string.Empty;
            ...
            try
            {
                sConnString = _sqlObject.GetClientDBConnString(pEntContModule.ClientId);
                sqlConnection = new SqlConnection(sConnString);
                _sqlcmd = new SqlCommand(Schema.ContentModule.PROC_CHECK_CONTENT_PATH, sqlConnection);
        ...
        private ContentModule FillLockAssessmentCourse(SqlDataReader pSqlReader)
        {
            ...
            if (pSqlReader.HasRows)
            {
                ...
                iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_FIRST_FAILED_ATTEMPT_LOCKHRS);
                if (!pSqlReader.IsDBNull(iIndex))
                    entContentModule.FirstFailedAttemptLockHrs = pSqlReader.GetInt32(iIndex);

                iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_ADMIN_UNLOCKHRS);
                if (!pSqlReader.IsDBNull(iIndex))
                    entContentModule.AdminUnlockHrs = pSqlReader.GetInt32(iIndex);

                iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_LOCK_ASSESSMENT);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.LockAssessment = pSqlReader.GetBoolean(iIndex);

                iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_LOCK_ATTEMPT_ASSESSMENT);
                if (!pSqlReader.IsDBNull(iIndex))
                    entContentModule.LockAttemptAssessment = pSqlReader.GetBoolean(iIndex);
            }
        ...
        private ContentModule FillObject(SqlDataReader pSqlReader, bool pRangeList)
        {
            ...
            if (pSqlReader.HasRows)
            {
                ...
                if (_sqlObject != null && _sqlObject.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_IS_MASTERCONTENT))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_IS_MASTERCONTENT);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.IsMasterContent = pSqlReader.GetBoolean(iIndex);
                }
                ...
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_RAPIDEL_COURSEID))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_RAPIDEL_COURSEID);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.RapidelCourseId = pSqlReader.GetString(iIndex);
                }
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_PERUSE_COURSEID))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_PERUSE_COURSEID);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.PeruseCourseId = pSqlReader.GetString(iIndex);
                }

                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_COURSE_ASSESSMENT_FLAG))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_COURSE_ASSESSMENT_FLAG);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.CouseAssessmentFlag = pSqlReader.GetBoolean(iIndex);
                }

                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_LOCK_ASSESSMENT))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_LOCK_ASSESSMENT);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.LockAssessment = pSqlReader.GetBoolean(iIndex);
                ...
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_FIRST_FAILED_ATTEMPT_LOCKHRS))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_FIRST_FAILED_ATTEMPT_LOCKHRS);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.FirstFailedAttemptLockHrs = pSqlReader.GetInt32(iIndex);
                ...
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_ADMIN_UNLOCKHRS))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_ADMIN_UNLOCKHRS);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.AdminUnlockHrs = pSqlReader.GetInt32(iIndex);
                ...
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_SEND_EMAIL_TO_LEARNER))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_SEND_EMAIL_TO_LEARNER);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.SendEmailToLearner = pSqlReader.GetBoolean(iIndex);
                }

                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_SENDEMAIL))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_SENDEMAIL);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.SendEmail = pSqlReader.GetBoolean(iIndex);
                }

                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_SENDEMAILTOMANAGER))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_SENDEMAILTOMANAGER);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.SendEmailToManager = pSqlReader.GetBoolean(iIndex);
                }
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_SEND_EMAIL_TO_REGIONALADMIN))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_SEND_EMAIL_TO_REGIONALADMIN);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.SendEmailToRegionalAdmin = pSqlReader.GetBoolean(iIndex);
                }
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_SENDEMAILTOMORE))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_SENDEMAILTOMORE);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.SendEmailToMore = pSqlReader.GetBoolean(iIndex);
                }

                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_SENDEMAILTO))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_SENDEMAILTO);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.SendEmailTo = pSqlReader.GetString(iIndex);
                }

                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_SHOW_ASSESSMENT_RESULT))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_SHOW_ASSESSMENT_RESULT);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.ShowAssessmentResult = pSqlReader.GetString(iIndex);
                }

                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_LOCK_ATTEMPT_ASSESSMENT))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_LOCK_ATTEMPT_ASSESSMENT);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.LockAttemptAssessment = pSqlReader.GetBoolean(iIndex);
                ...
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_AFTER_NUMBER_OF_ATTEMPTS))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_AFTER_NUMBER_OF_ATTEMPTS);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.AfterNumberofAttempts = pSqlReader.GetInt32(iIndex);
                }

                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_THUMBNAIL_IMG_PATH))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_THUMBNAIL_IMG_PATH);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.ThumbnailImgRelativePath = pSqlReader.GetString(iIndex);
                }
                if (pRangeList)
                {
        ...
        private ContentModule FillObject_CoursePlayer(SqlDataReader pSqlReader, bool pRangeList)
        {
            ...
            if (pSqlReader.HasRows)
            {
                ...
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_MASTERY_SCORE))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_MASTERY_SCORE);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.MasteryScore = pSqlReader.GetInt32(iIndex);
                }
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_ALLOW_RESIZE))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_ALLOW_RESIZE);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.AllowResize = pSqlReader.GetBoolean(iIndex);
                }
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_ALLOW_SCROLL))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_ALLOW_SCROLL);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.AllowScroll = pSqlReader.GetBoolean(iIndex);
                }
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_COURSE_LAUNCH_NEW_WIND0W))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_COURSE_LAUNCH_NEW_WIND0W);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.CourseLaunchNewWindow = pSqlReader.GetBoolean(iIndex);
                }
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_COURSE_LAUNCH_SAME_WIND0W))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_COURSE_LAUNCH_SAME_WIND0W);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.CourseLaunchSameWindow = pSqlReader.GetBoolean(iIndex);
                ...
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_COURSE_WINDOW_HEIGHT))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_COURSE_WINDOW_HEIGHT);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.CourseWindowHeight = pSqlReader.GetInt32(iIndex);
                }
                if (SQLHelper.ReaderHasColumn(pSqlReader, Schema.ContentModule.COL_COURSE_WINDOW_WIDTH))
                {
                    iIndex = pSqlReader.GetOrdinal(Schema.ContentModule.COL_COURSE_WINDOW_WIDTH);
                    if (!pSqlReader.IsDBNull(iIndex))
                        entContentModule.CourseWindowWidth = pSqlReader.GetInt32(iIndex);
        ...
        //update course details ...27/10/2015
        public ContentModule UpdateCourseDetails(ContentModule pEntContModule)
        {  
            ...
            try
            {
                sConnString = _sqlObject.GetClientDBConnString(pEntContModule.ClientId);
                sqlConnection = new SqlConnection(sConnString);
                _sqlcmd = new SqlCommand(Schema.ContentModule.PROC_UPDATE_CONTENT_COURSE_DETAILS, sqlConnection);
                _sqlpara = new SqlParameter(Schema.ContentModule.PARA_RAPIDEL_COURSE_ID, pEntContModule.RapidelCourseId);
                _sqlcmd.Parameters.Add(_sqlpara);
                ...
                if (obj != null)
                {
                    entContModule.ID = Convert.ToString(obj);
                }
        ...
        private ContentModule Update(ContentModule pEntContModule, string pStrUpdateMode)
        {
            ...
            if (!string.IsNullOrEmpty(pEntContModule.RapidelCourseId))
                _sqlpara = new SqlParameter(Schema.ContentModule.PARA_RAPIDEL_COURSE_ID, pEntContModule.RapidelCourseId);
            else
                _sqlpara = new SqlParameter(Schema.ContentModule.PARA_RAPIDEL_COURSE_ID, null);
            _sqlcmd.Parameters.Add(_sqlpara);
            ...
            _sqlpara = new SqlParameter(Schema.ContentModule.PARA_COURSE_ASSESSMENT_FLAG, pEntContModule.CouseAssessmentFlag);
            _sqlcmd.Parameters.Add(_sqlpara);

            _sqlpara = new SqlParameter(Schema.ContentModule.PARA_LOCK_ASSESSMENT, pEntContModule.LockAssessment);
            _sqlcmd.Parameters.Add(_sqlpara);

            _sqlpara = new SqlParameter(Schema.ContentModule.PARA_FIRST_FAILED_ATTEMPT_LOCKHRS, pEntContModule.FirstFailedAttemptLockHrs);
            _sqlcmd.Parameters.Add(_sqlpara);

            _sqlpara = new SqlParameter(Schema.ContentModule.PARA_ADMIN_UNLOCKHRS, pEntContModule.AdminUnlockHrs);
            _sqlcmd.Parameters.Add(_sqlpara);

            _sqlpara = new SqlParameter(Schema.ContentModule.PARA_SEND_EMAIL_TO_LEARNER, pEntContModule.SendEmailToLearner);
            _sqlcmd.Parameters.Add(_sqlpara);

            _sqlpara = new SqlParameter(Schema.ContentModule.PARA_SENDEMAIL, pEntContModule.SendEmail);
            _sqlcmd.Parameters.Add(_sqlpara);

            _sqlpara = new SqlParameter(Schema.ContentModule.PARA_SENDEMAILTOMANAGER, pEntContModule.SendEmailToManager);
            _sqlcmd.Parameters.Add(_sqlpara);

            _sqlpara = new SqlParameter(Schema.ContentModule.PARA_SENDE_MAIL_TO_REGIONALADMIN, pEntContModule.SendEmailToRegionalAdmin);
            _sqlcmd.Parameters.Add(_sqlpara);

            _sqlpara = new SqlParameter(Schema.ContentModule.PARA_SENDEMAILTOMORE, pEntContModule.SendEmailToMore);
            _sqlcmd.Parameters.Add(_sqlpara);

            if (!string.IsNullOrEmpty(pEntContModule.SendEmailTo))
                _sqlpara = new SqlParameter(Schema.ContentModule.PARA_SENDEMAILTO, pEntContModule.SendEmailTo);
            else
                _sqlpara = new SqlParameter(Schema.ContentModule.PARA_SENDEMAILTO, null);
            _sqlcmd.Parameters.Add(_sqlpara);

            if (!string.IsNullOrEmpty(pEntContModule.ShowAssessmentResult))
                _sqlpara = new SqlParameter(Schema.ContentModule.PARA_SHOW_ASSESSMENT_RESULT, pEntContModule.ShowAssessmentResult);
            else
                _sqlpara = new SqlParameter(Schema.ContentModule.PARA_SHOW_ASSESSMENT_RESULT, null);
            _sqlcmd.Parameters.Add(_sqlpara);

            _sqlpara = new SqlParameter(Schema.ContentModule.PARA_LOCK_ATTEMPT_ASSESSMENT, pEntContModule.LockAttemptAssessment);
            _sqlcmd.Parameters.Add(_sqlpara);

            _sqlpara = new SqlParameter(Schema.ContentModule.PARA_AFTER_NUMBER_OF_ATTEMPTS, pEntContModule.AfterNumberofAttempts);
            _sqlcmd.Parameters.Add(_sqlpara);

            if (!String.IsNullOrEmpty(pEntContModule.ThumbnailImgRelativePath))
                _sqlpara = new SqlParameter(Schema.ContentModule.PARA_THUMBNAILIMGRELATIVEPATH, pEntContModule.ThumbnailImgRelativePath);
            else
                _sqlpara = new SqlParameter(Schema.ContentModule.PARA_THUMBNAILIMGRELATIVEPATH, null);
            _sqlcmd.Parameters.Add(_sqlpara);
        ...
        public List<ContentModule> GetContentModuleList(ContentModule pEntContentModule)
        {
            ...
            try
            {
                ...
                //Add Bharat: 18-Sep-2015
                if (pEntContentModule.CreatedById != null)
                {
                    _sqlpara = new SqlParameter(Schema.Common.PARA_CREATED_BY, pEntContentModule.CreatedById);
                    _sqlcmd.Parameters.Add(_sqlpara);
                }
                _sqlreader = _sqlObject.SqlDataReader(_sqlcmd, false);
        ...
        public List<ContentModule> AddAllModules(List<ContentModule> pEntListContentModule)
        {
            ...
            try
            {
                if (pEntListContentModule.Count > 0)
                {
                    ...
                    _dtable.Columns.Add(Schema.ContentModule.COL_IS_HTML_5);
                    _dtable.Columns.Add(Schema.ContentModule.COL_KEEP_ZIP_FILE);
                    //Add Lock Course Assessment
                    _dtable.Columns.Add(Schema.ContentModule.COL_COURSE_ASSESSMENT_FLAG);
                    _dtable.Columns.Add(Schema.ContentModule.COL_LOCK_ASSESSMENT);
                    _dtable.Columns.Add(Schema.ContentModule.COL_FIRST_FAILED_ATTEMPT_LOCKHRS);
                    _dtable.Columns.Add(Schema.ContentModule.COL_ADMIN_UNLOCKHRS);
                    _dtable.Columns.Add(Schema.ContentModule.COL_SEND_EMAIL_TO_LEARNER);
                    _dtable.Columns.Add(Schema.ContentModule.COL_SENDEMAIL);
                    _dtable.Columns.Add(Schema.ContentModule.COL_SENDEMAILTOMANAGER);
                    _dtable.Columns.Add(Schema.ContentModule.COL_SEND_EMAIL_TO_REGIONALADMIN);
                    _dtable.Columns.Add(Schema.ContentModule.COL_SENDEMAILTOMORE);
                    _dtable.Columns.Add(Schema.ContentModule.COL_SENDEMAILTO);
                    _dtable.Columns.Add(Schema.ContentModule.COL_SHOW_ASSESSMENT_RESULT);

                    _dtable.Columns.Add(Schema.ContentModule.COL_LOCK_ATTEMPT_ASSESSMENT);
                    _dtable.Columns.Add(Schema.ContentModule.COL_AFTER_NUMBER_OF_ATTEMPTS);
                    ...
                    foreach (ContentModule entCM in pEntListContentModule)
                    {
                        ...
                        if (_dtable.Select(Schema.ContentModule.COL_CONTENT_MODULE_ID + "='" + entCM.ID + "'").Length <= 0)
                        {
                            ...
                            {
                                if (pEntListContentModule.Count == 1)
                                    _dtable.Rows[_dtable.Rows.Count - 1][Schema.ContentModule.COL_DEL_LANGUAGES] = true;
                                else
                                    _dtable.Rows[_dtable.Rows.Count - 1][Schema.ContentModule.COL_DEL_LANGUAGES] = 1;
                                iLangCount = 0;
                            }
                            ...
                            if (pEntListContentModule.Count == 1)
                                ...
                                drow[Schema.ContentModule.COL_IS_ACTIVE] = Convert.ToBoolean(entCM.IsActive) ? 1 : 0;

                            //if (pEntListContentModule.Count == 1)
                            //    drow[Schema.ContentModule.COL_IS_UPLOADED] = Convert.ToBoolean(entCM.IsUploaded);
                            //else
                            //    drow[Schema.ContentModule.COL_IS_UPLOADED] = Convert.ToBoolean(entCM.IsUploaded) ? 1 : 0;

                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_IS_UPLOADED] = entCM.IsUploaded;
                            else
                                drow[Schema.ContentModule.COL_IS_UPLOADED] = entCM.IsUploaded ? 1 : 0;
                            ...
                            if (pEntListContentModule.Count == 1)
                                ...
                                drow[Schema.ContentModule.COL_SCORE_TRACKING] = entCM.ScoreTracking ? 1 : 0;

                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_QUESTION_RESPONSE_TRACKING] = entCM.QuestionResponseTracking;
                            else
                                drow[Schema.ContentModule.COL_QUESTION_RESPONSE_TRACKING] = entCM.QuestionResponseTracking ? 1 : 0;
                            ...
                            if (pEntListContentModule.Count == 1)
                                ...
                                drow[Schema.ContentModule.COL_COURSE_LAUNCH_SAME_WIND0W] = entCM.CourseLaunchSameWindow ? 1 : 0;

                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_COURSE_LAUNCH_NEW_WIND0W] = entCM.CourseLaunchNewWindow;
                            else
                                drow[Schema.ContentModule.COL_COURSE_LAUNCH_NEW_WIND0W] = entCM.CourseLaunchNewWindow ? 1 : 0;

                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_ALLOW_SCROLL] = entCM.AllowScroll;
                            else
                                drow[Schema.ContentModule.COL_ALLOW_SCROLL] = entCM.AllowScroll ? 1 : 0;
                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_ALLOW_RESIZE] = entCM.AllowResize;
                            else
                                drow[Schema.ContentModule.COL_ALLOW_RESIZE] = entCM.AllowResize ? 1 : 0;
                            ...
                            if (pEntListContentModule.Count == 1)
                                ...
                                drow[Schema.ContentModule.COL_IS_PRINT_CERTIFICATE] = entCM.IsPrintCertificate ? 1 : 0;

                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_IS_COURSE_SESSION_NO_EXPIRY] = entCM.IsCourseSessionNoExpiry;
                            else
                                drow[Schema.ContentModule.COL_IS_COURSE_SESSION_NO_EXPIRY] = entCM.IsCourseSessionNoExpiry ? 1 : 0;

                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_IS_SHORT_LANGUAGE_CODE] = entCM.IsShortLanguageCode;
                            else
                                drow[Schema.ContentModule.COL_IS_SHORT_LANGUAGE_CODE] = entCM.IsShortLanguageCode ? 1 : 0;
                            ...
                            if (pEntListContentModule.Count == 1)
                                ...
                                drow[Schema.ContentModule.COL_ISASSESSMENT] = entCM.IsAssessment ? 1 : 0;

                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_ISMIDDLEPAGE] = entCM.IsMiddlePage;
                            else
                                drow[Schema.ContentModule.COL_ISMIDDLEPAGE] = entCM.IsMiddlePage ? 1 : 0;

                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_IS_HTML_5] = entCM.IsHTML5;
                            else
                                drow[Schema.ContentModule.COL_IS_HTML_5] = entCM.IsHTML5 ? 1 : 0;

                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_KEEP_ZIP_FILE] = entCM.KeepZipFile;
                            else
                                drow[Schema.ContentModule.COL_KEEP_ZIP_FILE] = entCM.KeepZipFile ? 1 : 0;
                            //Add Course Assessment lock
                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_COURSE_ASSESSMENT_FLAG] = entCM.CouseAssessmentFlag;
                            else
                                drow[Schema.ContentModule.COL_COURSE_ASSESSMENT_FLAG] = entCM.CouseAssessmentFlag ? 1 : 0;
                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_LOCK_ASSESSMENT] = entCM.LockAssessment;
                            else
                                drow[Schema.ContentModule.COL_LOCK_ASSESSMENT] = entCM.LockAssessment ? 1 : 0;
                            drow[Schema.ContentModule.COL_FIRST_FAILED_ATTEMPT_LOCKHRS] = entCM.FirstFailedAttemptLockHrs;
                            drow[Schema.ContentModule.COL_ADMIN_UNLOCKHRS] = entCM.AdminUnlockHrs;
                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_SEND_EMAIL_TO_LEARNER] = entCM.SendEmailToLearner;
                            else
                                drow[Schema.ContentModule.COL_SEND_EMAIL_TO_LEARNER] = entCM.SendEmailToLearner ? 1 : 0;

                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_SENDEMAIL] = entCM.SendEmail;
                            else
                                drow[Schema.ContentModule.COL_SENDEMAIL] = entCM.SendEmail ? 1 : 0;
                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_SENDEMAILTOMANAGER] = entCM.SendEmailToManager;
                            else
                                drow[Schema.ContentModule.COL_SENDEMAILTOMANAGER] = entCM.SendEmailToManager ? 1 : 0;
                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_SEND_EMAIL_TO_REGIONALADMIN] = entCM.SendEmailToRegionalAdmin;
                            else
                                drow[Schema.ContentModule.COL_SEND_EMAIL_TO_REGIONALADMIN] = entCM.SendEmailToRegionalAdmin ? 1 : 0;
                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_SENDEMAILTOMORE] = entCM.SendEmailToMore;
                            else
                                drow[Schema.ContentModule.COL_SENDEMAILTOMORE] = entCM.SendEmailToMore ? 1 : 0;
                            drow[Schema.ContentModule.COL_SENDEMAILTO] = entCM.SendEmailTo;
                            drow[Schema.ContentModule.COL_SHOW_ASSESSMENT_RESULT] = entCM.ShowAssessmentResult;

                            if (pEntListContentModule.Count == 1)
                                drow[Schema.ContentModule.COL_LOCK_ATTEMPT_ASSESSMENT] = entCM.LockAttemptAssessment;
                            else
                                drow[Schema.ContentModule.COL_LOCK_ATTEMPT_ASSESSMENT] = entCM.LockAttemptAssessment ? 1 : 0;
                            drow[Schema.ContentModule.COL_AFTER_NUMBER_OF_ATTEMPTS] = entCM.AfterNumberofAttempts;

                            drow[Schema.ContentModule.COL_CONTENT_MODULE_DESC] = entCM.ContentModuleDescription ?? (object)DBNull.Value;
                            drow[Schema.ContentModule.COL_KEY_WORDS] = entCM.ContentModuleKeyWords ?? (object)DBNull.Value;
                    ...
                    if (_dtable.Rows.Count > 0)
                    {
                        ...
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_CONTENT_MODULE_ENGLISH_NAME, SqlDbType.NVarChar, 100, Schema.ContentModule.COL_CONTENT_MODULE_ENGLISH_NAME);
                        ...
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_ISHTML5, SqlDbType.Bit, 1, Schema.ContentModule.COL_IS_HTML_5);
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_KEEPZIPFILE, SqlDbType.Bit, 1, Schema.ContentModule.COL_KEEP_ZIP_FILE);

                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_COURSE_ASSESSMENT_FLAG, SqlDbType.Bit, 1, Schema.ContentModule.COL_COURSE_ASSESSMENT_FLAG);
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_LOCK_ASSESSMENT, SqlDbType.Bit, 1, Schema.ContentModule.COL_LOCK_ASSESSMENT);
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_FIRST_FAILED_ATTEMPT_LOCKHRS, SqlDbType.Int, 10, Schema.ContentModule.COL_FIRST_FAILED_ATTEMPT_LOCKHRS);
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_ADMIN_UNLOCKHRS, SqlDbType.Int, 10, Schema.ContentModule.COL_ADMIN_UNLOCKHRS);

                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_SEND_EMAIL_TO_LEARNER, SqlDbType.Bit, 1, Schema.ContentModule.COL_SEND_EMAIL_TO_LEARNER);
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_SENDEMAIL, SqlDbType.Bit, 1, Schema.ContentModule.COL_SENDEMAIL);
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_SENDEMAILTOMANAGER, SqlDbType.Bit, 1, Schema.ContentModule.COL_SENDEMAILTOMANAGER);
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_SENDE_MAIL_TO_REGIONALADMIN, SqlDbType.Bit, 1, Schema.ContentModule.COL_SEND_EMAIL_TO_REGIONALADMIN);
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_SENDEMAILTOMORE, SqlDbType.Bit, 1, Schema.ContentModule.COL_SENDEMAILTOMORE);

                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_SENDEMAILTO, SqlDbType.VarChar, 1000, Schema.ContentModule.COL_SENDEMAILTO);
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_SHOW_ASSESSMENT_RESULT, SqlDbType.VarChar, 50, Schema.ContentModule.COL_SHOW_ASSESSMENT_RESULT);

                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_LOCK_ATTEMPT_ASSESSMENT, SqlDbType.Bit, 1, Schema.ContentModule.COL_LOCK_ATTEMPT_ASSESSMENT);
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_AFTER_NUMBER_OF_ATTEMPTS, SqlDbType.Int, 10, Schema.ContentModule.COL_AFTER_NUMBER_OF_ATTEMPTS);

                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_CONTENT_MODULE_DESC, SqlDbType.NVarChar, -1, Schema.ContentModule.COL_CONTENT_MODULE_DESC);
                        _sqladapter.InsertCommand.Parameters.Add(Schema.ContentModule.PARA_KEY_WORDS, SqlDbType.NVarChar, 500, Schema.ContentModule.COL_KEY_WORDS);
        ...
        /// <returns></returns>
        public List<YPLMS2._0.API.Entity.ContentModule> FindContentModule(Search pEntSearch)
        {
            _sqlObject = new SQLObject();
            YPLMS2._0.API.Entity.ContentModule entContModule = null;
            ...
            if (!String.IsNullOrEmpty(pEntSearch.KeyWord))
                _sqlpara = new SqlParameter(Schema.Common.PARA_KEY_WORD, pEntSearch.KeyWord.Trim());
            else
            ...
            if (pEntSearch.SearchObject != null && pEntSearch.SearchObject.Count > 0)
            {
                ...
                entContModule = JsonSerializer.Deserialize<ContentModule>(pEntSearch.SearchObject[0].ToString());
            ...
            {
                _expCustom = new CustomException(_strMessageId, CustomException.WhoCallsMe(), ExceptionSeverityLevel.Critical, expCommon, true, sqlMasterConnection.ConnectionString);
                throw _expCustom;
            }
            finally
            {
        ...
        public ContentModule GetByIdForCoursePlayer(string clientId, string courseId)
        {
            return GetContentModuleByID_CoursePlayer(new ContentModule { ClientId = clientId, ID = courseId });
        }
