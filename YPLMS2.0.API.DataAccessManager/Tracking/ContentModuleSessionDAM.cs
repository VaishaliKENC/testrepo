...
{
    public class ContentModuleSessionDAM : DataAccess, IContentModuleSessionDAM<ContentModuleSession>
    {
        ...
        public ContentModuleSession SaveSession( ContentModuleSession session)
        {
            ...
            if (session.Expiration == DateTime.MinValue)
            {
                expirationParameter.Value = DateTime.UtcNow; // Set today's UTC date if default DateTime value
            }
            else
            {
                expirationParameter.Value = session.Expiration; // Use the actual value if it's not the default
            }


            session.SessionId = ExecuteScalar<Guid>(storedProcedure, session.ClientId, contentModuleParameter, userParameter,
