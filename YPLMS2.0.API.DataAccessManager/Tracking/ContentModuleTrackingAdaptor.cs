...
namespace YPLMS2._0.API.DataAccessManager
{
    ...
    public class ContentModuleTrackingAdaptor : IContentModuleTrackingAdaptor, IDataManager<ContentModuleTracking>,IContentModuleTrackingAdaptor<ContentModuleTracking>
    {
        #region Declaration
        ...
        string _strMessageId = YPLMS.Services.Messages.Content.CONT_MOD_TRACK_ERROR;
        ...
        {
            ContentModuleTrackingUpdateResult result;
            try
            {
        ...
        private ContentModuleTrackingUpdateResult Update(ContentModuleTracking pEntContModTracking, string pStrUpdateMode)
        {
            ...
            if (pEntContModTracking.IsScorm2004 == true)
                sqlcmd.CommandText = Schema.ContentModuleTracking.PROC_UPDATE_CONTENT_MODULE_TRACKING_Scorm2004;
            else
            ...
            {
                pEntContModTracking.ID = Schema.Common.VAL_ATTEMPT_ID_PREFIX + YPLMS.Services.IDGenerator.GetUniqueKey(Schema.Common.VAL_ATTEMPT_ID_LENGTH);
            }
            ...
            //// Chnages by Prajakta 
            if (pEntContModTracking.CustomNoOfPagesCompleted != 0)
                sqlpara = new SqlParameter(Schema.ContentModuleTracking.PARA_CUSTOM_TOTAL_NO_OF_PAGES, pEntContModTracking.CustomTotalNoOfPages);
            else
                sqlpara = new SqlParameter(Schema.ContentModuleTracking.PARA_CUSTOM_TOTAL_NO_OF_PAGES, null);
            sqlcmd.Parameters.Add(sqlpara);

            if (pEntContModTracking.CustomNoOfPagesCompleted != 0)
                sqlpara = new SqlParameter(Schema.ContentModuleTracking.PARA_CUSTOM_NO_OF_PAGES_COMPLETED, pEntContModTracking.CustomNoOfPagesCompleted);
            else
                sqlpara = new SqlParameter(Schema.ContentModuleTracking.PARA_CUSTOM_NO_OF_PAGES_COMPLETED, null);
            sqlcmd.Parameters.Add(sqlpara);
            //// End Prajakta
        ...
        //UpdateAssessmentCourse
        public ContentModuleTracking UpdateAssessmentCourse(ContentModuleTracking pEntContentModuleTracking)
        {
            _sqlObject = new SQLObject();
            sqlcmd = new SqlCommand();
            sqlcmd.CommandText = Schema.ContentModuleTracking.PROC_UPDATE_ASSESSMENTCOURSE_TRACKING;
            try
            {
