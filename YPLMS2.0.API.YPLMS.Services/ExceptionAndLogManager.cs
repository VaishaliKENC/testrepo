...
namespace YPLMS2._0.API.YPLMS.Services
{
    public class ExceptionManager
    {
        ...
        public static string WriteError(Exception pExc, string pEventID, short pCategoryID, string pOtherInfo, string pClientConnectionString)
        {
            string strErrCode = "";
            ...
            //        {
            //            strErrCode = WriteErrorToDataBase(pExc, pEventID, pCategoryID, pOtherInfo, pClientConnectionString);
            //            break;
            //        }
        ...
        public static string WriteErrorToFile(string pLog)
        {
            ...
            if (_strFilPthForErr == "" || _strLogNameForErr == null)
            {
                try
                {
                    ...
                    if (!Directory.Exists("C:\\Temp\\CoursePlayerLog"))
                    {
                        Directory.CreateDirectory("C:\\Temp\\CoursePlayerLog");
                    }
                    _strFilPthForErr = "C:\\Temp\\CoursePlayerLog\\CoursePlayerLog.txt";
                }
                catch (Exception ExcFc)
                {
                }
        ...
        /// <param name="pLog"></param>
        /// <returns></returns>
        public static string Log(string pLog)
        {
            string strErrCode = string.Empty;

            if (_strLog == "" || _strLog == null)
            {
                try
                {
                    if (!Directory.Exists("C:\\Revamp"))
                    {
                        Directory.CreateDirectory("C:\\Revamp");
                    }
                    if (!Directory.Exists("C:\\Revamp\\Log"))
                    {
                        Directory.CreateDirectory("C:\\Revamp\\Log");
                    }
                    _strLog = "C:\\Revamp\\Log\\Log.txt";
                }
                catch (Exception Ex)
                {
                }
            ...
            try
            {
                StreamWriter swFile = new StreamWriter(_strLog, true);
                swFile.WriteLine(pLog);
                swFile.Close();
            }
            catch (Exception Ex)
        ...
        /// <param name="pOtherInfo"></param>
        /// <returns></returns>
        private static string WriteErrorToDataBase(Exception pExc, string pEventID, short pCategoryID, string pOtherInfo,string pClientConnectionString)
        {
            ...
            int iRowIns = ExecuteCommand(sqlCmd, pClientConnectionString);
            return string.Empty;
        }
        /// <summary>
        /// ExecuteCommand 
        ...
        /// <returns></returns>
        public static int ExecuteCommand(SqlCommand pCommand,string pClientConnectionString)
        {
            ...
            try
            {
                if(!string.IsNullOrEmpty(ConnectionStringForError))
                {                    
                ...
                {
                    sqlcon = new SqlConnection(pClientConnectionString);
                }
