...
function BVScorm_getAPI() {
    ...
    if (myAPI == null && window.parent) myAPI = findAPI(window.parent)   
    if (myAPI == null && window.top) myAPI = findAPI(window.top);
    if (myAPI == null && window.opener) if (window.opener != null && !window.opener.closed) myAPI = findAPI(window.opener);      
    if (myAPI == null && window.opener){
    	 if (myAPI == null && window.opener.parent) myAPI = findAPI(window.opener.parent)
...
function BVScorm_getscore() {
	...
	if (BVScorm_checkIfAPIAvailable()) {
		//*-*----------------------------------------------
		if(scormVersion=="scorm_2004"){			
			score = BVScorm_API.GetValue("cmi.score.scaled",score/100);
		} 
		if(scormVersion=="scorm_1.2"){		
			score = BVScorm_API.LMSGetValue("cmi.core.score.raw", score); 
		}
	}
	return score;
}
...
function BVScorm_GetUserName(){
	...
	if (BVScorm_checkIfAPIAvailable()) {
		try{
			...
			if(scormVersion=="scorm_1.2"){	
				retVal = BVScorm_API.GetValue("cmi.core.student_name");		
			}
		} catch(e){
		}
	}
